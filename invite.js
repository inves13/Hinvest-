...
const checkAuth = () => {
  firebase.auth().onAuthStateChanged(user => {
    if (user) {
      ...
      loadInviteInfo(user.uid); // Carrega as informações de convite quando o usuário está logado
    } else {
...
const searchUsersByCodigoIndicacao = (codigoIndicacao) => {
  ...
  usuariosRef.orderByChild('codigoIndicacao').equalTo(codigoIndicacao).once('value', (snapshot) => {
    if (snapshot.exists()) {
      ...
      invitedList.innerHTML = ''; // Limpar a lista antes de adicionar os novos indicados
...
const registerUser = (nome, telefone, senha, codigoIndicacao = "") => {
  firebase.auth().createUserWithEmailAndPassword(telefone + '@domain.com', senha)
    .then((userCredential) => {
      ...
      firebase.database().ref('usuarios/' + user.uid).set({
        ...
        saldo: 20,
        saldoConta: 10,
        codigoIndicacao: codigoIndicacao,  // Código de indicação do convidado
        timestamp: Date.now(),
        ultimoCheckin: "",
      }).then(() => {
